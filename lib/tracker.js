Meteor.tracker = {
    ...
    ,
    _analyzeTrack: function (trackParse) {
        var regex = /#[^\s]+\b/g;
        var track = trackParse.track.match(regex);

        if (track) {
            trackParse.trackData.track = track[0].substring(1);
    ...
    _analyzeDate: function (trackParse) {
        ...
        //2016-1-5,2016-01-05,16-1-5,16-01-05
        ...
        if (dates.length) {
            ...
            trackDate.year = parsedDate.getFullYear();
            trackDate.month = parsedDate.getMonth();
            trackDate.day = parsedDate.getDate();

            trackParse.trackData.date = moment(trackDate).toDate();
    ...
    analyzeTrack: function (track) {
        ...
        Meteor.tracker._analyzeTrack(trackParse);
