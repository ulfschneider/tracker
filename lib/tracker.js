Meteor.tracker = {
    ...
    _analyzeComment: function (analyze) {
        ...
        if (comment) {
            analyze.chartData.comment = comment[0].substring(2);
    ...
    _analyzeTrack: function (analyze) {
        ...
        if (track) {
            analyze.chartData.track= track[0].substring(1);
    ...
    _analyzeDuration: function (analyze) {
        ...
        if (duration) {
            analyze.chartData.duration = Meteor.tracker._sumMillis(duration);
    ...
    _analyzeResults: function (analyze) {
        ...
        if (results) {
            analyze.chartData.results = results;
    ...
    analyzeTrack: function (track) {
        ...
        if (moment(analyze.chartData.date).isAfter(new Date())) {
            analyze.errors.push({
                code: Meteor.tracker.ERR_FUTURE_DATE,
                description: "Track date in the future is not allowed: " + moment(analyze.chartData.date).format("ddd YYYY-MM-DD HH:mm")
            });
        }
